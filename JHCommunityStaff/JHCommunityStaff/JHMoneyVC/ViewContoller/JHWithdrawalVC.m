...
@interface JHWithdrawalVC ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation JHWithdrawalVC

- (void)viewDidLoad {
    [super viewDidLoad];
    [self addTitle:NSLocalizedString(@"提现明细", nil)];
...
- (void)loadNewData{
    ...
    [HttpTool postWithAPI:@"staff/money/txlog" withParams:dic success:^(id json) {
        NSLog(@"json%@",json);
...
- (void)loadMoreData{
    ...
    [HttpTool postWithAPI:@"staff/money/txlog" withParams:dic success:^(id json) {
        ...
        [_tableView.mj_footer endRefreshing];
        NSLog(@"eror%@",error.localizedDescription);
        [self showAlertViewWithTitle:NSLocalizedString(@"抱歉,无法连接服务器", nil)];
    }];
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if(section == 0){
        ...
        label.text = NSLocalizedString(@"最近30天资金提现明细", nil);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString *identifier = @"withdraw";
    JHWithdrawCell *cell = [_tableView dequeueReusableCellWithIdentifier:identifier];
    if(cell == nil){
        cell = [[JHWithdrawCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
