...
#import "XHCodeTF.h"
@interface JHForgetPasswordVC ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate>
{
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self addTitle:NSLocalizedString(@"找回密码", nil)];
    [self initSubViews];
...
- (void)initSubViews{
    ...
    _passwordAgain.placeholder = NSLocalizedString(@"请再次确认新密码(不少于6位)", nil);
...
- (void)clickCertainBnt{
    ...
    if(_mobile.text.length == 0){
        ...
        SHOW_HUD
        NSDictionary *dic = @{@"mobile":_mobile.text,@"sms_code":_code.text,@"passwd":_password.text};
        [HttpTool postWithAPI:@"staff/entry/forgot" withParams:dic success:^(id json) {
            NSLog(@"json%@",json);
            if([json[@"error"] isEqualToString:@"0"]){
                HIDE_HUD
                [self.navigationController popToViewController:self.navigationController.viewControllers[0] animated:YES];
                [[NSNotificationCenter defaultCenter] postNotificationName:@"FindPassword" object:nil];
                [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"token"];
                [[NSUserDefaults standardUserDefaults] synchronize];
            }else{
                HIDE_HUD
                [self showAlertViewWithTitle:[NSString stringWithFormat:NSLocalizedString(@"找回密码失败,原因:%@", nil),json[@"message"]]];
            }
        } failure:^(NSError *error) {
            HIDE_HUD
            NSLog(@"error%@",error.localizedDescription);
