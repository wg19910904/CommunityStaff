...
#import "HttpTool.h"
#import "MJRefresh.h"
@interface JHCountOrderVC ()<UITableViewDataSource,UITableViewDelegate>
{
    UITableView *_tableView;
    NSMutableArray *_orderArray;//订单
...
- (void)requestData{
    SHOW_HUD
    [HttpTool postWithAPI:@"staff/tongji/order" withParams:@{} success:^(id json) {
        NSLog(@"json%@",json);
        if([json[@"error"] isEqualToString:@"0"]){
            HIDE_HUD
            [_noNetBackView removeFromSuperview];
            _monthDic = json[@"data"][@"month_arr"];
            _weekDic = json[@"data"][@"week_arr"];
            [_orderArray addObject:json[@"data"][@"today_orders"]];
            [_orderArray addObject:json[@"data"][@"week_orders"]];
            [_orderArray addObject:json[@"data"][@"month_orders"]];
            [_orderArray addObject:json[@"data"][@"total_orders"]];
            _isYes = YES;
            [self createTableView];
        }else{
            HIDE_HUD
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    switch (indexPath.section) {
        case 0:
        {
           ...
           [cell setDataArray:_orderArray withType:@"order"];
            return cell;
        }
            break;
        case 1:
        {
            ...
            [cell configUIWthType:@"order" data:_weekDic[@"value"] title:_weekDic[@"key"]];
            return cell;
        }
            break;
        case 2:
        {
            ...
            [cell creatNSMutableArray:_monthDic[@"value"] withNSMutableArray:dateArray withType:@"order"];
            return cell;
        }
            break;
        default:
