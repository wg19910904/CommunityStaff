...
#import "XHCodeTF.h"
@interface JHMaintainApplyVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
{
...
@implementation JHMaintainApplyVC
...
- (void)initSubViews{
    ...
    NSString *string = NSLocalizedString(@"我已认真阅读《维修师傅入驻协议》", nil);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    switch (indexPath.section) {
        ...
        case 5:
        {
            JHMaintainSkillCell *cell = [[JHMaintainSkillCell alloc] init];
            cell.dataArray = _dataArray;
            [cell setMaintainSkillBlock:^(NSArray *skillDataArray) {
...
- (void)clickApplyBnt{
    ...
    if(_name.text.length == 0){
        ...
        [self showAlertViewWithTitle:NSLocalizedString(@"请选择维修技能", nil)];
    }else if (_checkBnt.selected == NO){
        [self showAlertViewWithTitle:NSLocalizedString(@"请勾选并阅读维修师傅入驻协议", nil)];
