...
#import "JHHouseSkillCell.h"
#import "SecurityCode.h"
#import "XHCodeTF.h"
@interface JHHouseApplyVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
{
    ...
    NSMutableArray *_skillDataArray;//技能数组作为参数
    NSMutableArray *_dataArray;//技能数据源
    UIView *_noNetBackView;//没有网络是背景图
    NSInteger maxNum;
    SecurityCode *_control;
}
@end

@implementation JHHouseApplyVC
...
#pragma mark====获取网络技能数据======
- (void)getSkillData{
    SHOW_HUD
    NSDictionary *dic = @{@"from":@"house"};
    [HttpTool postWithAPI:@"staff/staff/cate" withParams:dic success:^(id json) {
        NSLog(@"json%@",json);
        if([json[@"error"] isEqualToString:@"0"]){
            ...
            [_tableView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:5]] withRowAnimation:UITableViewRowAnimationNone];
        }else{
            HIDE_HUD
...
- (void)initSubViews{
    ...
    NSString *string = NSLocalizedString(@"我已认真阅读《家政阿姨入驻协议》", nil);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    switch (indexPath.section) {
        ...
        case 5:
        {
            JHHouseSkillCell *cell = [[JHHouseSkillCell alloc] init];
            cell.dataArray = _dataArray;
            [cell setHouseSkillBlock:^(NSArray *skillDataArray) {
                _skillDataArray = [skillDataArray mutableCopy];
            }];
            return cell;
        }
            break;
...
- (void)clickApplyBnt{
    ...
    if(_name.text.length == 0){
    ...
    }else if(_skillDataArray.count == 0){
        [self showAlertViewWithTitle:NSLocalizedString(@"请选择家政技能", nil)];
    }else if (_checkBnt.selected == NO){
        [self showAlertViewWithTitle:NSLocalizedString(@"请勾选并阅读家政阿姨入驻协议", nil)];
    }else if(_passwordAgain.text.length < 6 || _password.text.length < 6){
        ...
        NSDictionary *dic = @{@"from":@"house",@"uname":_name.text,@"mobile":_mobile.text,@"sms_code":_code.text,@"passwd":_password.text,@"city_id":[JHShareModel shareModel].cityCode,@"options":dataArray,@"open_id":(open_id ? open_id : @""),@"register_id":(register_id ? register_id : @""),@"lat":[NSString stringWithFormat:@"%f",baiduLat],@"lng":[NSString stringWithFormat:@"%f",baiduLng]};
