...
static char MAPointAnnotation_img;
...
- (UIImage *)img{
    return objc_getAssociatedObject(self, &MAPointAnnotation_img);
}

-(void)setImg:(UIImage *)img{
    
    objc_setAssociatedObject(self, &MAPointAnnotation_img,img,OBJC_ASSOCIATION_COPY_NONATOMIC);
}
...
- (void)mapView:(MAMapView *)mapView didUpdateUserLocation:(MAUserLocation *)userLocation updatingLocation:(BOOL)updatingLocation{
    ...
    [XHMapKitManager shareManager].lat = self.user_lat;
    [XHMapKitManager shareManager].lng = self.user_lng;
    [self getCurrentLocationName:CLLocationCoordinate2DMake(self.user_lat, self.user_lng)];
}
...
/**
 ...
 @param location 位置
 */
-(void)getCurrentLocationName:(CLLocationCoordinate2D)location{
...
- (void)onReGeocodeSearchDone:(AMapReGeocodeSearchRequest *)request response:(AMapReGeocodeSearchResponse *)response
{
    if(response.regeocode != nil)
    {
        ...
        if (tempArr.count && tempArr1.count) {
            if (self.clickBlock) {
                self.clickBlock([tempArr1[0] name],[tempArr[0] name]);
            }
        }else{
            if (self.clickBlock) {
                self.clickBlock(@"",@"");
...
- (MAAnnotationView *)mapView:(MAMapView *)mapView viewForAnnotation:(id <MAAnnotation>)annotation{
    if ([annotation isKindOfClass:[MAPointAnnotation class]]) {
        ...
        annotationView.image = ann.img?ann.img:[UIImage imageNamed:imgStr];
...
- (MAOverlayRenderer *)mapView:(MAMapView *)mapView rendererForOverlay:(id <MAOverlay>)overlay{
    ...
    polylineView.lineWidth = 5.f;
    polylineView.strokeColor = _lineColor?_lineColor:HEX(@"ff6600", 1.0f);
    polylineView.lineJoinType =   kCGLineJoinRound;//连接类型
    polylineView.lineCapType = kCGLineCapButt;//端点类型
    polylineView.lineDashType = _lineDashType;
    return polylineView;
...
-(void)addLineWithStartPoint:(CLLocationCoordinate2D)start
    ...
    _lineDashType = lineDashType;
    _lineColor = lineColor;
    NSArray *latArr = @[@(start.latitude).stringValue,@(end.latitude).stringValue];
    NSArray *lngArr = @[@(start.longitude).stringValue,@(end.longitude).stringValue];
