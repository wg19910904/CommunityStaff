...
#pragma mark=====初始化子控件=======
- (void)initSubViews{
  UIImageView *img = [[UIImageView alloc] initWithFrame:FRAME(0, 7.5, 60, 25)];
    img.image = IMAGE(@"order_labelbg");
...
- (void)setMaintainModel:(JHMaintainListModel *)maintainModel{
    _maintainModel = maintainModel;
    _typeLbel.text = maintainModel.cate_title;
    _typeLbel.adjustsFontSizeToFitWidth = YES;
    _basic.text = [NSString stringWithFormat:@"%@ %@",maintainModel.contact,maintainModel.mobile];
    _status.text = maintainModel.order_status_label;
    _orderId.text = [NSString stringWithFormat:NSLocalizedString(@"订单ID:%@", nil),maintainModel.order_id];
    _orderTime.text = [NSString stringWithFormat:NSLocalizedString(@"下单时间:%@", nil),[TransformTime transfromWithString:maintainModel.dateline]];
    _destination.text = [NSString stringWithFormat:NSLocalizedString(@"目的地:%@%@", nil),maintainModel.addr,maintainModel.house];
    _distant.text = maintainModel.juli_label;
    if(maintainModel.intro.length == 0){
        _note.text = NSLocalizedString(@"备注: (无)", nil);
    }else{
        _note.text = [NSString stringWithFormat:NSLocalizedString(@"备注:%@", nil),maintainModel.intro];
    }
    _money.text = [NSString stringWithFormat:NSLocalizedString(@"定金:￥%@", nil),maintainModel.danbao_amount];
...
- (void)handleStatusButton{
    if([_maintainModel.order_status isEqualToString:@"0"] ){
    ...
    }else if([_maintainModel.order_status isEqualToString:@"1"]  ||  [_maintainModel.order_status isEqualToString:@"2"]){
        _statusButton.userInteractionEnabled = YES;
        [_statusButton setTitleColor:THEME_COLOR forState:UIControlStateNormal];
    ...
    }else if([_maintainModel.order_status isEqualToString:@"3"]){
        _statusButton.userInteractionEnabled = YES;
        [_statusButton setTitleColor:THEME_COLOR forState:UIControlStateNormal];
    ...
    }else if ([_maintainModel.order_status isEqualToString:@"4"]){
        _statusButton.userInteractionEnabled = YES;
        [_statusButton setTitleColor:THEME_COLOR forState:UIControlStateNormal];
    ...
    }else if ([_maintainModel.order_status isEqualToString:@"5"]){
    ...
    }else if ([_maintainModel.order_status isEqualToString:@"8"] && [_maintainModel.comment_info[@"comment_id"] isEqualToString:@"0"]){
        _statusButton.userInteractionEnabled = NO;
        [_statusButton setTitleColor:HEX(@"cccccc", 1.0f) forState:UIControlStateNormal];
        ...
        if([_maintainModel.comment_info[@"reply_time"] isEqualToString:@"0"]){
            _statusButton.userInteractionEnabled = YES;
            [_statusButton setTitleColor:THEME_COLOR forState:UIControlStateNormal];
