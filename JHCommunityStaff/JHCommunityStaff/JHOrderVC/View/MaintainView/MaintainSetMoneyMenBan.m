...
#import "HttpTool.h"
@interface MaintainSetMoneyMenBan ()<UITextFieldDelegate>
{
...
@implementation MaintainSetMoneyMenBan
+ (void)creatSetMoneyMenBanWithOrderId:(NSString *)order_id viewController:(JHBaseVC *)vc success:(void (^)(void))success cancel:(void (^)(void))cancel{
    MaintainSetMoneyMenBan *setMoney = [[MaintainSetMoneyMenBan alloc] init];
...
- (void)creatSetMoneyMenBan:(MaintainSetMoneyMenBan *)menBan{
...
- (void)certainMoneyButton{
    ...
    [HttpTool postWithAPI:@"staff/weixiu/order/setprice" withParams:dic success:^(id json) {
        NSLog(@"json%@",json);
        if([json[@"error"] isEqualToString:@"0"]){
         ...
         [[NSNotificationCenter defaultCenter] postNotificationName:@"maintainSetMoney" object:nil];
        }else{
            [MBProgressHUD hideHUDForView:_backView animated:YES];
            [_setMoneyMenBan removeFromSuperview];
            _setMoneyMenBan = nil;
