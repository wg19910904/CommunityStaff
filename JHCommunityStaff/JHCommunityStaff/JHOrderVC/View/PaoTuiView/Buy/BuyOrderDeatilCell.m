...
@interface BuyOrderDeatilCell ()<   AVAudioPlayerDelegate>
{
    UIView *_imgBackView;//图片白色背景
    DisplayImageInView *_dispalyView;//展示图片
...
@end
@implementation BuyOrderDeatilCell
...
- (void)setPaotuiFrameModel:(PaoTuiOrderDetailFrameModel *)paotuiFrameModel{
    _paotuiFrameModel = paotuiFrameModel;
     _orderID.text = [NSString stringWithFormat:NSLocalizedString(@"订单ID:%@", nil),paotuiFrameModel.paoTuiDetailModel.order_id];
    _time.text = [NSString stringWithFormat:NSLocalizedString(@"收货时间:%@", nil),[TransformTime transfromWithString:paotuiFrameModel.paoTuiDetailModel.pei_time]];
    if(paotuiFrameModel.paoTuiDetailModel.intro.length == 0){
        _note.text = NSLocalizedString(@"备注: (无)", nil);
    }else{
        _note.text = [NSString stringWithFormat:NSLocalizedString(@"备注:%@", nil),paotuiFrameModel.paoTuiDetailModel.intro];
    }
    _note.frame = _paotuiFrameModel.buyNoteRect;
    _recordBck.frame = FRAME(15, 65 + _note.frame.size.height + 10, 100, 25);
    ...
    _imgBackView.frame = FRAME( 15, _paotuiFrameModel.buyOrderDetailHeight - 15 - (WIDTH - 60) / 4, WIDTH - 30, (WIDTH - 60) / 4);
    ...
    _bottomLine.frame = FRAME(0, _paotuiFrameModel.buyOrderDetailHeight - 0.5, WIDTH, 0.5);
