...
@property (nonatomic,strong)UILabel *buyAddr;//购货地址
@property (nonatomic,strong)UILabel *deliverAddr;//收货地址
@property (nonatomic,strong)UILabel *buyDistant;//购货距离
...
@property (nonatomic,strong)UIImageView *assignImg;//是否指定
@end
@implementation JHBuyListCell
...
- (void)initSubViews{
    ...
    _typeLbel.text = NSLocalizedString(@"帮我买", nil);
    ...
    UILabel *buyLabel = [[UILabel alloc] initWithFrame:FRAME(40, 45 + 40 + 35, 55, 15)];
    ...
    _buyAddr = [[UILabel alloc] initWithFrame:FRAME(130, 45 + 40 + 35, WIDTH - 130 - 95, 15)];
    _buyAddr.textColor = HEX(@"666666", 1.0f);
    _buyAddr.font = FONT(12);
    [self.contentView addSubview:_buyAddr];
    _assignImg = [[UIImageView alloc] initWithFrame:FRAME(95, 45 + 40 + 35, 30, 15)];
    ...
    _buyDistant = [[UILabel alloc] initWithFrame:FRAME(WIDTH - 95, 45 + 40 + 35, 80, 15)];
    _buyDistant.textColor = HEX(@"ff6600", 1.0f);
    _buyDistant.font = FONT(12);
    _buyDistant.textAlignment = NSTextAlignmentRight;
...
- (void)setPaotuiListModel:(JHPaotuiListModel *)paotuiListModel{
    ...
    if(_paotuiListModel.o_addr.length == 0){
        _assignImg.image = IMAGE(@"order_label_appointno");
        _buyDistant.text = @"";
        _deliverDistant.text = _paotuiListModel.juli_quancheng;
    }else{
        _assignImg.image = IMAGE(@"order_label_appoint");
        _buyDistant.text = _paotuiListModel.juli_qidian;
        _deliverDistant.text = _paotuiListModel.juli_zhongdian;
    }
    _buyAddr.text = [NSString stringWithFormat:@"%@%@",_paotuiListModel.o_addr,_paotuiListModel.o_house];
    _deliverAddr.text = [NSString stringWithFormat:NSLocalizedString(@"收货地址:%@%@", nil),paotuiListModel.addr,paotuiListModel.house];
...
- (void)handleOrderStatusButton{
    ...
    }else if ([_paotuiListModel.order_status isEqualToString:@"5"]||[_paotuiListModel.order_status isEqualToString:@"4"]){
