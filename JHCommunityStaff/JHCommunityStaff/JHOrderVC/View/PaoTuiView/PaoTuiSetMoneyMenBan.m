...
#import "HttpTool.h"
@interface PaoTuiSetMoneyMenBan ()<UITextFieldDelegate>
{
...
@implementation PaoTuiSetMoneyMenBan
+ (void)creatSetMoneyMenBanWithOrderId:(NSString *)order_id viewController:(JHBaseVC *)vc success:(void (^)(void))success cancel:(void(^)(void))cancel{
    PaoTuiSetMoneyMenBan *setMoney = [[PaoTuiSetMoneyMenBan alloc] init];
...
- (void)creatSetMoneyMenBan:(PaoTuiSetMoneyMenBan *)menBan{
    ...
    _amountFiled.placeholder = NSLocalizedString(@"请输入商品小票金额", nil);
    ...
    describel.text = NSLocalizedString(@"金额为您购买商品的金额", nil);
...
- (void)certainMoneyButton{
    ...
    [HttpTool postWithAPI:@"staff/paotui/order/setprice" withParams:dic success:^(id json) {
        NSLog(@"json%@",json);
        if([json[@"error"] isEqualToString:@"0"]){
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:@"paotuiSetMoney" object:nil];
        }else{
            [MBProgressHUD hideHUDForView:_backView animated:YES];
            [_setMoneyMenBan removeFromSuperview];
            _setMoneyMenBan = nil;
