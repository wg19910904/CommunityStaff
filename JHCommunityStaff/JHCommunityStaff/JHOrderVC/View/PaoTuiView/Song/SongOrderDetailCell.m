...
@interface SongOrderDetailCell ()<AVAudioPlayerDelegate>
{
    UILabel *_orderID;//订单ID
    UILabel *_pickTime;//取货时间
    UILabel *_deliverTime;//收货时间
    UILabel *_note;//备注
...
@end
@implementation SongOrderDetailCell
...
- (void)initSubViews{
    ...
    _pickTime = [[UILabel alloc] initWithFrame:FRAME(15, 40 , WIDTH  - 30, 15)];
    _pickTime.textColor = HEX(@"666666", 1.0f);
    _pickTime.font = FONT(12);
    [self.contentView addSubview:_pickTime];
    _deliverTime = [[UILabel alloc] initWithFrame:FRAME(15, 65 , WIDTH  - 30, 15)];
    _deliverTime.textColor = HEX(@"666666", 1.0f);
    _deliverTime.font = FONT(12);
    [self.contentView addSubview:_deliverTime];
...
- (void)setPaotuiFrameModel:(PaoTuiOrderDetailFrameModel *)paotuiFrameModel{
    ...
    _pickTime.text = [NSString stringWithFormat:NSLocalizedString(@"取货时间:%@", nil),[TransformTime transfromWithString:_paotuiFrameModel.paoTuiDetailModel.o_time]];;
    _deliverTime.text = [NSString stringWithFormat:NSLocalizedString(@"收货时间:%@", nil),[TransformTime transfromWithString:_paotuiFrameModel.paoTuiDetailModel.time]];
    ...
    _note.frame = _paotuiFrameModel.songNoteRect;
    _recordBck.frame =  FRAME(15, 90 + _note.frame.size.height + 10, 100, 25);
    _recordSecond.frame = FRAME(120, 105 + _note.frame.size.height , 100, 15);
    ...
    _imgBackView.frame = FRAME( 15, _paotuiFrameModel.songOrderDetailHeight - 15 - (WIDTH - 60) / 4, WIDTH - 30, (WIDTH - 60) / 4);
    ...
    _bottomLine.frame = FRAME(0, _paotuiFrameModel.songOrderDetailHeight - 0.5, WIDTH, 0.5);
}
#pragma mark ==========播放录音=====================
