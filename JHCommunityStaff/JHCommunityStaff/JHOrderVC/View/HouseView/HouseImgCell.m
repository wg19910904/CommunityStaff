...
- (void)setHouseOrderDetailFrameModel:(JHHouseOrderDetailFrameModel *)houseOrderDetailFrameModel{
    ...
    for(int i = 0; i < houseOrderDetailFrameModel.houseDetailModel.photos.count; i ++){
        UIImageView *img = [[UIImageView alloc] initWithFrame:FRAME(15 + ((WIDTH - 60) / 4  + 10) * i, 10,  (WIDTH - 60) / 4, (WIDTH - 60) / 4)];
        [img sd_setImageWithURL:[NSURL URLWithString:[IMAGEADDRESS stringByAppendingString:houseOrderDetailFrameModel.houseDetailModel.photos[i]]] placeholderImage:IMAGE(@"pj_pic")];
...
- (void)tapImg:(MyTapGesture *)sender{
    if(_displayView == nil){
        ...
        [_displayView showInViewWithImageUrlArray:_houseOrderDetailFrameModel.houseDetailModel.photos withIndex:sender.tag withBlock:^{
