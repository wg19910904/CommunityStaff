...
@interface JHSongOrderDetailVC ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation JHSongOrderDetailVC
...
- (void)addNotifiCation{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(requestData) name:@"songMapStatusButton" object:nil];
}
...
- (void)initSubViews{
    ...
    _typeLabel.text = NSLocalizedString(@"帮我送", nil);
...
- (void)requestWitApi:(NSString *)api dic:(NSDictionary *)dic alert:(NSString *)alert{
    ...
    [HttpTool postWithAPI:api withParams:dic success:^(id json) {
        ...
        if([json[@"error"] isEqualToString:@"0"]){
            ...
            [self requestData];
            [[NSNotificationCenter defaultCenter] postNotificationName:@"songDetailStatusButton" object:nil];
        }else{
            HIDE_HUD
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section == 0){
        if(indexPath.row == 0)
            ...
            return _paotuiFrameModel.songOrderDetailHeight;
    }else if (indexPath.section == 1){
        if(indexPath.row == 0)
            return 40;
        else
            return _paotuiFrameModel.songCustomerMessageHeight;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if(indexPath.section == 0){
        ...
        if(indexPath.row == 0){
            ...
            SongOrderDetailCell *cell = [[SongOrderDetailCell alloc] init];
        ...
        if(indexPath.row == 0){
            ...
            title.font = FONT(14);
            title.textColor = HEX(@"333333", 1.0f);
            title.text = NSLocalizedString(@"取货&收货信息", nil);
            [cell.contentView addSubview:title];
            cell.contentView.backgroundColor = [UIColor whiteColor];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            for(int i = 0; i < 2; i ++){
            ...
            SongCustomerMessageCell *cell = [[SongCustomerMessageCell alloc] init];
            [cell.pickMobile addTarget:self action:@selector(clickPickerMobile) forControlEvents:UIControlEventTouchUpInside];
            [cell.deliverMobile addTarget:self action:@selector(clickDeliverMobile) forControlEvents:UIControlEventTouchUpInside];
            [cell setPaotuiFrameModel:_paotuiFrameModel];
            return cell;
        }
    }else if(indexPath.section == 2){
        if (indexPath.row == 0) {
            ...
            if ([_paotuiDetaiModel.hongbao floatValue] == 0) {
                amountLab.text =  NSLocalizedString(@"未使用红包", NSStringFromClass([self class]));
            }else{
                amountLab.text = [NSString stringWithFormat:@"-¥%@",_paotuiDetaiModel.hongbao];
            }
            return cell;
        }
    }else if(indexPath.section == 3){
...
#pragma mark======取货人打电话点击事件======
- (void)clickPickerMobile{
    
    [self showMobile:_paotuiDetaiModel.o_mobile];
}
#pragma mark====收货人电话点击事件======
- (void)clickDeliverMobile{
...
- (void)clickLookButton{
    ...
    double lat = 0.00;
    double lng = 0.00;
    [GaoDe_Convert_BaiDu transform_baidu_to_gaodeWithBD_lat:[_paotuiDetaiModel.o_lat doubleValue] WithBD_lon:[_paotuiDetaiModel.o_lng doubleValue] WithGD_lat:&o_lat WithGD_lon:&o_lng];
